import{d as O}from"./characterPopup-P2n-s9fy.js";function h(){return typeof window<"u"&&!!window.electronAPI}async function A(){if(h()&&window.electronAPI&&window.electronAPI.ping){const e=await window.electronAPI.ping();return console.log("Ping result from main process:",e),e}else return console.log("Not running in Electron."),null}function B(e,a){const o=document.createElement("div");o.id="voice-loading-overlay",o.innerHTML=`
  ...
  `,document.head.appendChild(g);const t=o.querySelector("#voice-loading-title"),r=o.querySelector("#voice-loading-subtitle"),l=o.querySelector("#voice-loading-action");let n=!1;async function i(){if(!n){n=!0,l.disabled=!0,t.textContent="Preparing narrator voice...",r.textContent="Please wait while audio is being initialized.";try{if(h()&&window.electronAPI&&window.electronAPI.loadGamePage){await window.electronAPI.loadGamePage();return}o.remove(),g.remove(),await a()}catch{n=!1,l.disabled=!1,t.textContent="Unable to initialize narrator voice.",r.textContent="Press the button again to retry audio unlock."}}}l.addEventListener("click",i),t.style.display=""}document.addEventListener("DOMContentLoaded",()=>{B("./assets/HPG2_title-DyN6F_xP.png",async()=>{})});const m=[getComputedStyle(document.documentElement).getPropertyValue("--c-blue").trim(),getComputedStyle(document.documentElement).getPropertyValue("--c-red").trim(),getComputedStyle(document.documentElement).getPropertyValue("--c-orange").trim(),getComputedStyle(document.documentElement).getPropertyValue("--c-yellow").trim(),getComputedStyle(document.documentElement).getPropertyValue("--c-green").trim()];function w(e){const a=String(e).trim().replace(/^#/,""),o=parseInt(a,16);return{r:o>>16&255,g:o>>8&255,b:o&255}}let p=null,d=w(m[1]),y=2%m.length,s=0;const M=.02;window.fastSkipEnabled=!1;const E="hpg2-dialogue-audio-enabled",S="hpg2-color-option-enabled",I="hpg2-fast-skip-enabled";function T(){try{return localStorage.getItem(S)==="true"}catch{return!1}}function v(e){try{localStorage.setItem(S,String(e))}catch{}}function z(){try{return localStorage.getItem(I)==="true"}catch{return!1}}function F(e){try{localStorage.setItem(I,String(e))}catch{}}window.fastSkipEnabled=z();function N(){try{return localStorage.getItem(E)==="true"}catch{return!1}}function H(e){try{localStorage.setItem(E,String(e))}catch{}}window.dialogueAudioEnabled=N();function k(){const e=w(m[y]);Math.round(d.r+(e.r-d.r)*s),Math.round(d.g+(e.g-d.g)*s),Math.round(d.b+(e.b-d.b)*s),s+=M,s>=1&&(s=0,d=e,y=(y+1)%m.length)}function D(e){e&&e.stopPropagation();const a=document.getElementById("color-btn");p?(clearInterval(p),p=null,c(a,"Change color",!1),v(!1)):(p=setInterval(k,50),c(a,"Change color",!0),v(!0))}function c(e,a,o){e.innerHTML=`<span class="settings-option-label">${a}</span><span class="settings-option-sep">: </span><span class="settings-option-state ${o?"is-on":"is-off"}">${o?"ON":"OFF"}</span>`}document.getElementById("color-btn").addEventListener("click",D);function _(e){e&&e.stopPropagation();const a=document.getElementById("fast-skip-btn");window.fastSkipEnabled=!window.fastSkipEnabled,c(a,"Fast skip",window.fastSkipEnabled),F(window.fastSkipEnabled)}document.getElementById("fast-skip-btn").addEventListener("click",_);const u=document.getElementById("audio-toggle-btn"),j='<svg class="audio-icon" viewBox="0 0 24 24" aria-hidden="true"><polygon points="3,10 7,10 12,6 12,18 7,14 3,14"></polygon><path d="M16 9 C18 10.5,18 13.5,16 15"></path><path d="M18.8 6.5 C22.2 9.3,22.2 14.7,18.8 17.5"></path></svg>',q='<svg class="audio-icon" viewBox="0 0 24 24" aria-hidden="true"><polygon points="3,10 7,10 12,6 12,18 7,14 3,14"></polygon><line x1="5" y1="5" x2="19" y2="19"></line></svg>';function C(){const e=!!window.dialogueAudioEnabled;u.innerHTML=e?j:q,u.title=`Audio: ${e?"ON":"OFF"}`,u.setAttribute("aria-label",`Audio: ${e?"ON":"OFF"}`)}function G(e){e&&e.stopPropagation(),window.dialogueAudioEnabled=!window.dialogueAudioEnabled,H(window.dialogueAudioEnabled),C()}u.addEventListener("click",G);C();const L=document.getElementById("settings-btn"),f=document.getElementById("settings-menu"),P=document.getElementById("settings-overlay");function b(e){f.classList.toggle("show",e),P.classList.toggle("show",e)}L.addEventListener("click",()=>{const e=!f.classList.contains("show");b(e)});P.addEventListener("click",()=>{b(!1)});document.addEventListener("click",e=>{f.classList.contains("show")&&!f.contains(e.target)&&e.target!==L&&e.target!==u&&b(!1)});const x=document.getElementById("color-btn");T()?(p=setInterval(k,50),c(x,"Change color",!0)):c(x,"Change color",!1);const $=document.getElementById("fast-skip-btn");c($,"Fast skip",window.fastSkipEnabled);const V=document.getElementById("voice-loading-card");O(window,V);document.getElementById("voice-loading-action").addEventListener("click",function(){sessionStorage.setItem("fromStartMenu","1"),sessionStorage.removeItem("mainChara"),sessionStorage.removeItem("saveFile"),sessionStorage.removeItem("startYear"),window.location.href="hpg2-main.html"});document.getElementById("voice-loading-load").addEventListener("click",async function(){let e=document.createElement("div");e.id="loadgame-overlay",e.style.position="fixed",e.style.inset="0",e.style.background="rgba(8,8,16,0.85)",e.style.zIndex="6000",e.style.display="flex",e.style.alignItems="center",e.style.justifyContent="center",e.innerHTML=`<div id="loadgame-box" style="background:rgb(20,20,30);border-radius:12px;padding:28px 24px;min-width:320px;max-width:90vw;box-shadow:0 0 20px #000;border:2px solid #444;position:relative;">
      ...
      </div>`,document.body.appendChild(e),e.addEventListener("mousedown",function(t){t.target===e&&e.remove()});const a=e.querySelector("#loadgame-list");let o=null;fetch("scripts/savelist.json").then(t=>t.json()).then(t=>{if(!Array.isArray(t)||!t.length){a.innerHTML='<div style="color:#bbb;">No save files found.</div>';return}function r(){a.innerHTML="",t.forEach(l=>{const n=document.createElement("button");n.textContent=l,n.style.display="block",n.style.width="100%",n.style.margin="6px 0",n.style.padding="10px",n.style.fontFamily="monospace",n.style.fontSize="15px",n.style.background=o===l?"#282860":"#282830",n.style.color=o===l?"#fff":"#e8e8f0",n.style.border=o===l?"2px solid #b4b4dc":"2px solid #444",n.style.borderRadius="8px",n.style.cursor="pointer",n.addEventListener("click",i=>{i.stopPropagation(),o=l,r(),g()}),a.appendChild(n)})}r()}).catch(()=>{a.innerHTML='<div style="color:#bbb;">No save files found.</div>'});function g(){let t=document.createElement("div");t.style.position="fixed",t.style.left="0",t.style.top="0",t.style.width="100vw",t.style.height="100vh",t.style.background="rgba(0,0,0,0.25)",t.style.display="flex",t.style.alignItems="center",t.style.justifyContent="center",t.style.zIndex="7000",t.innerHTML=`<div style='background:rgb(30,30,40);padding:32px 28px;border-radius:12px;box-shadow:0 0 16px #000;border:2px solid #888;min-width:220px;text-align:center;'>

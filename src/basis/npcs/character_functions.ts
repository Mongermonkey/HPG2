...
/**
 * Helper functions for managing NPCs and their relationships.
 */

import * as u from '../../utilities/_index';
import { WheelSegment } from '../../utilities/_index';
import { Character, characterList } from './characters';
import { MainChara } from '../main_character/maincharacter';
import { hogwartsHouseName, hogwartsRole } from '../types/base_types';
import { shiftAlignment } from '../main_character/character_development';
...
{
    let result = await u.spinWheel('Who do you befriend?', getStudentList(chara, sameHouse, senior));
...
export async function improveConnection(chara: MainChara<'Wizard'>, friend: Character<hogwartsRole> | string): Promise<void>
{
    ...
    {
        await u.showText('Your relation with ' + friend.longname + ' has improved.');
    ...
    switch (studentFriend?.connectionlvl)
    {
        case 'foe':
            ...
            await u.showText('You and ' + studentFriend.longname + ', who used to despise you, unexpectedly became friends. Cheers!');
            break;
        case 'none':
        case 'neutral':
            studentFriend.connectionlvl = 'friend';
            await u.showText('You befriended ' + studentFriend.longname + ' (' + studentFriend.house + ')!');
            break;
        case 'friend':
            studentFriend.connectionlvl = 'bff';
            await u.showText('You and ' + studentFriend.longname + ' are now BFFs! Cheers!');
            break;
        case 'bff':
            studentFriend.connectionlvl = 'lover';
            await u.showText('Something changed between you and ' + studentFriend.longname + '.\nAfter what you\'ve been through, you and ' + studentFriend.name + ' became lovers!');
            break;
    }
    ...
    if (['Harry', 'Ron', 'Hermione'].includes(studentFriend.name))
    {
        ...
        {
            await u.showText(studentFriend.name + ' introduced you to his friend ' + harry!.longname + ' (Gryffindor).');
            harry!.connectionlvl = 'friend';
        }
        if (studentFriend.longname !== 'Ron Weasley')
        {
            await u.showText(studentFriend.name + ' introduced you to his friend ' + ron!.longname + ' (Gryffindor).');
            ron!.connectionlvl = 'friend';
        }
        if (studentFriend.longname !== 'Hermione Granger')
        {
            await u.showText(studentFriend.name + ' introduced you to his friend ' + hermione!.longname + ' (Gryffindor).');
    ...
    else if (studentFriend.longname === 'Draco Malfoy')
    {
        ...
        if (!isFriend(crabbe))
        {
            await u.showText('Draco introduced you to his friend ' + crabbe!.longname + ' (Slytherin).');
            crabbe!.connectionlvl = 'friend';
        }
        if (!isFriend(goyle))
        {
            await u.showText('Draco introduced you to his friend ' + goyle!.longname + ' (Slytherin).');
    ...
    if (['Fred Weasley', 'George Weasley', 'Padma Patil', 'Parvati Patil'].includes(studentFriend.longname))
    {
        ...
        await u.showText('You got to know ' + studentFriend.name + '\'s ' + (studentFriend.name === 'Fred' || studentFriend.name === 'George Weasley' ? 'twin brother' : 'twin sister') + ' ' + other.name + ' as well!');
    }
}
...
{
    await u.showText('Your relation with ' + foe.longname + ' has worsened.');
    ...
    if (foe.longname === 'Draco Malfoy' && foe.connectionlvl === 'foe')
    {
        ...
        await u.showText('Vincent Crabbe and Gregory Goyle, Draco\'s closest friends, are also upset with you for what happened with Draco.\nYour relation with them has worsened as well.');
    }
    ...
    if ((foe.longname === 'Fred Weasley' || foe.longname === 'George Weasley') && foe.connectionlvl === 'foe')
    {
        ...
        await u.showText('Your relation with ' + otherName + ' has worsened as well.');
    }
...
export function getStudentList(chara: MainChara<'Wizard'>, sameHouse: boolean = false, senior?: boolean): WheelSegment[]
{
    ...
    return students.map(s => u.newSegment(s.longname, fraction));
}

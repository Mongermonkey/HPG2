...
import { QuidditchGame } from '../../basis/types/complex_types';
import { MainChara, Character, hogwartsHouseName, quidditchRole } from '../../basis/_index';
...
{
    let headname = b.getHeadOfHouse(chara.characterList, chara.house).longname;
    let captain = b.getQuidditchCaptain(chara.characterList, chara.house);

    if (chara.quidditchRole === 'candidate')
        await u.showText('As forwarned by ' + headname + ', you are now to attend the tryouts for the Quidditch team.');
    else
    {
        let chance = Math.min(95, 10 + b.getSkill(chara, 'Flying') * 10);
        let segments = [ u.newSegment('Interested', chance), u.newSegment('Not Interested', 100 - chance) ];

        let result = await u.spinWheel('Quidditch tryouts are now open! Interested in applying?', segments);
        if (result === 'Not Interested')
        {
            await u.showText('You decided not to try out for the Quidditch team this year.');
            return;
        }
    }

    await d.QuidditchSelection(captain.longname, chara.house);
    let pass = Math.min(95, b.getSkill(chara, 'Flying') * 10);

    let segments = [ u.newSegment('Pass', pass), u.newSegment('Fail', 100 - pass) ];
    let test = await u.spinWheel('Do you pass the Quidditch selection?', segments);

    if (test === 'Pass')
    {
        await u.showText('You passed the Quidditch selection!');
        await spinQuidditchRole(chara, captain);        
    }
    else await u.showText('Alas, you failed the Quidditch selection. Maybe you can try again next year.');
}
...
async function spinQuidditchRole(chara: MainChara<'Wizard'>, captain: Character<'Student'>): Promise<void>
{
    ...
    let segments =
    [
        u.newSegment('Seeker', seeker),
        u.newSegment('Chaser', chaser),
        u.newSegment('Beater', beater),
        u.newSegment('Keeper', keeper)
    ].filter(s => s.fraction > 0);

    let result = await u.spinWheel('What role will you play?', segments);
    chara.quidditchRole = result.toLowerCase() as quidditchRole;
    
    await u.showText('Congratulations! You have been selected as ' + result + ' for the ' + chara.house + ' Quidditch team!');
    await u.showText(captain.name + ' welcomes you to the team.');
    await b.improveConnection(chara, captain);
}
...
{
    await u.showText(gameIndex == 0 ? 'Today is the first Quidditch match of the year!'
    ...
    {
        await u.showText('Today\'s match is ' + game.houseA + ' vs ' + game.houseB + '.');
        if (!houseGame)
        {
            await u.showText('You have no particular interest in this match: you prefer to mind your business.');
            return;
        }        
        await u.showText('You watch eagerly as your house team plays!');
...
{
    if (houseGame) await u.showText('You are excited to play against ' + (game.houseA === chara.house ? game.houseB : game.houseA) + '!');
    else await u.showText('You go to see how ' + game.houseA + ' and ' + game.houseB + ' teams play.');

    setRandomScores(game);
    // Mc watching, not competing
    if (game.houseA !== chara.house && game.houseB !== chara.house) return await d.quidditchMatch(game.winner, game.loser);

    // If game.loser, skill-powered swap scores
    let skillFactor = Math.min(0.9, b.getSkill(chara, 'Flying') / 10);
    console.log('Chara flying skill: ' + b.getSkill(chara, 'Flying') + '\nskillFactor: ' + skillFactor);
    if(chara.house == game.loser && u.spinbool(skillFactor, 1 - skillFactor)) swapScores(game);
    
    await u.showText(chara.house != game.winner
...
async function postQuidditch(chara: MainChara<'Wizard'>, game: QuidditchGame): Promise<void>
{
    ...
    {
        await u.showText('All ' + chara.house + 's celebrates the victory together in the common room!');
...
async function quidditchTeamBond(chara: MainChara<'Wizard'>): Promise<void>
{
    ...
    let chance = u.newSegment('Make friends', friendshipChance * 100),
        notchance = u.newSegment('Mind your business', 100 - friendshipChance * 100);

    const result = await u.spinWheel('Friendship Wheel! What do you do?', [chance, notchance]);

    if (result === 'Mind your business')
    {
        await u.showText('You decide to mind your own business.');
        return;
    }

    await u.showText('You make friends!');
    await u.showText('Who do you befriend?');
    await b.befriend(chara, true);
}
